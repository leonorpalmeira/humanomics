...
def GenotypingAndRecalibrating(param):
    ...
    if param["AnalysisMode"]!="PANEL":
        ...
        sout+="""$BGZIP ${vcf/.vcf/_noCHR.vcf}\n"""
        sout+="""$TABIX -p vcf ${vcf/.vcf/_noCHR.vcf.gz}\n"""
        sout+="""\n"""
        ...
        sout+="""    -info EUR_AF \\\n"""
        sout+="""    -v $kgVCF \\\n"""
        sout+="""    ${vcf/.vcf/_noCHR.vcf.gz} | $BGZIP > ${vcf/.vcf/_noCHR_1KG.vcf.gz}\n"""
        sout+="""\n"""
        ...
        sout+="""    -info ExAC_AF \\\n"""
        sout+="""    -v $exacVCF \\\n"""
        sout+="""    ${vcf/.vcf/_noCHR.vcf_1KG.gz} | $BGZIP > ${vcf/.vcf/_noCHR_1KG_ExAC.vcf.gz}\n"""
        sout+="""\n"""
        sout+="""echo "************** Launching ClinVar Annotation ******************"\n"""
        sout+="""$SNPSIFT \\\n"""
        sout+="""    annotateMem \\\n"""
        sout+="""    -v $clinvarVCF \\\n"""
        sout+="""    ${vcf/.vcf/_noCHR_1KG_ExAC.vcf.gz} | $BGZIP > ${vcf/.vcf/_noCHR_1KG_ExAC_ClinVar.vcf.gz}\n"""
        sout+="""\n"""
    sout+="""echo "************** Finished ******************"\n"""
